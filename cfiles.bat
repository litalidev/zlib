@echo off
rem cfiles name version arch  build [tool tver] -s src1 [src2 ...]
rem cfiles zlib 1.2.11  x86   mtd   [v141 15.8] -s include lib\zlibstaticd.lib
rem cfiles %1   %2      %3    %4    %5
rem
rem name=project name, e.g. zlib
rem vesion=project version, e.g. 1.2.11
rem tool=platform tool version, v141=vs2017
rem ipath=temp installed path, e.g. p:\prj\libs\zlib\1.2.11\tmp
rem
rem filename: zlib-1.2.11-v141-15.8-x86-mdd.lib
rem filename: %name%-%version%-%tool%-%tver%-%arch%-%build%.lib
rem
rem
...
rem echo dext=%dext%
set dfname=%name%-%version%-%tool%-%tver%-%arch%-%build%%dext%
